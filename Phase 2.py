...
def createSuffix():
    ...
    global longest, longestInd, ansInd, par
    words = []
    startWords = []
    s = ""
    v = [] #Neighbourhood
...
def goFind():
    global RealOutput, leafNum, longest, longestInd, ansInd, rep, par, v
    #t = tPattern.get()
    Disable(tPattern)
    Disable(bFind)
    #print(t)
    ...
    leafNum = [0 for i in range(2*len(s) + 10)]
    longest, longestInd = 0, -1
    rep = int(tPattern.get())
    print("rep:", rep)
    ansInd = []
    phase2()


    print(longest, longestInd)
    if (longestInd == -1):
        print("No such thing")
        RealOutput = "No such thing"

    if longestInd != -1: 
        findAnsInd(longestInd)
    parents = []
    cur = longestInd
    ...
    for i in range(len(parents)-1, 0, -1):
        flag = False
        for u in range(len(v[parents[i]])):
            if v[parents[i]][u].to == parents[i-1]:
                anis += toStr(v[parents[i]][u].st, v[parents[i]][u].en)
                flag = True
                break


    print(anis)
    RealOutput = anis + "\nOccurred at: "+str(ansInd[0])
    for i in range (1, len(ansInd)):
        RealOutput += ", " + str(ansInd[i])
    print(ansInd)
...
title = Label(text = "Phase 2", font=("Courier", 18))
des = Label(text = "Finding the longest\nrepeating substring.", font=("Courier", 15))
...
t3 = Label(text = "Minimum number of repetition:") 
Disable(t3)
